...
epochs = 200
train_losses, val_losses, epoch_metrics = [], [], []
...
for epoch in range(epochs):
    ...
    with torch.no_grad():
        ...
        y_val_pred = val_preds.cpu().numpy()
    ...
    epoch_metrics.append([epoch + 1, loss.item(), val_loss.item(), precision, recall, f1])
...
metrics_df = pd.DataFrame([{
    ...
    'Epochs': len(train_losses),  # actual trained epochs before early stopping
    'Model': 'PyTorch_NN_with_BCEWithLogits'
}])

# Save to CSV
metrics_df.to_csv("nn_metrics_MoreEpochs.csv", index=False)
print("\nMetrics saved to nn_metrics_MoreEpochs.csv")
